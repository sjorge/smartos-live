...
function configValues(callback) {
    loadConfig(function (err, conf) {
        ...
        if (conf.hasOwnProperty('admin_tag')) {
        ...
        conf.vnics.forEach(function (nic) {
             ...
             */
            if ((tag != 'admin' && tag != 'external' && tag != admin_tag)
                || tagNums[tag] !== 0) {
        ...
        /* XXX: Since we set admin_tag above we shouldn't need to do this....

        // If the adminNic is specified override any admin values
        if (conf.hasOwnProperty('adminNic')
            && Object.keys(conf.adminNic).length !== 0) {

            if (conf.adminNic.ip && vals.admin_ip) {
                vals.admin_ip = conf.adminNic.ip;
            }

            if (conf.adminNic.mac && vals.admin_mac) {
                vals.admin_mac = conf.adminNic.mac;
            }

            if (conf.adminNic.netmask && vals.admin_netmask) {
                vals.admin_netmask = conf.adminNic.netmask;
            }

            if (conf.adminNic.mtu && vals.admin_mtu) {
                vals.admin_mtu = conf.adminNic.mtu;
            }

            if (conf.adminNic.mac && vals.admin_nic) {
